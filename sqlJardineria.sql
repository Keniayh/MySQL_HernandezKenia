...
-- ----------------Clausulas------------------------
-- LEFT JOIN, RIGHT JOIN, NATURAL LEFT JOIN y NATURAL RIGHT JOIN.
...
WHERE ciudad LIKE 'M%'
GROUP BY ciudad;
SELECT e.nombre, e.apellido1, e.apellido2, COUNT(c.codigo_cliente) AS num_clientes
FROM empleado e
...
WHERE EXISTS (
    SELECT 1
    FROM empleado ej
    WHERE e.codigo_jefe = ej.codigo_empleado
    AND ej.nombre = 'Alberto'
    AND ej.apellido1 = 'Soria'
);
SELECT e.nombre, e.apellido1, e.email
FROM empleado e
WHERE e.codigo_jefe = (
    SELECT ej.codigo_empleado
    FROM empleado ej
...
FROM detalle_pedido
GROUP BY codigo_pedido;
SELECT codigo_pedido, SUM(cantidad) AS suma_cantidad_productos
FROM detalle_pedido
GROUP BY codigo_pedido;
SELECT p.nombre, SUM(dp.cantidad) AS total_unidades_vendidas
FROM detalle_pedido dp
...
SELECT c.nombre_cliente,
       (SELECT COUNT(*)
        FROM pedido
        WHERE codigo_cliente = c.codigo_cliente) AS cantidad_pedidos
FROM cliente c;
SELECT c.nombre_cliente,
       (SELECT COALESCE(SUM(total), 0)
        FROM pago
        WHERE codigo_cliente = c.codigo_cliente) AS total_pagado
FROM cliente c;
...
WHERE EXISTS (
    ...
    AND YEAR(pedido.fecha_pedido) = 2008
)
